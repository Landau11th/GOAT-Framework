...
//optimization in 1D
double Conj_Grad_Min::OneD_Minimum(const arma::Col<double> start_coordinate, const arma::Col<double> search_direction_given)
{
    double lambda;
    static const double Golden_Ratio = 0.61803;
    const double step_size = 1.0;//sqrt(as_scalar(search_direction_given.t()*search_direction_given) ) * 1.0;
    const double epsilon_scaled = _epsilon/sqrt(arma::as_scalar(search_direction_given.t()*search_direction_given));

    //left, middle and right values
    double left, middle, right, next;
    left = middle = right = 0.0;
    //corresponding target function values
    double f_left, f_middle, f_right, f_next;

    middle = Golden_Ratio*step_size;
    right = step_size;

    f_left = Target_function(start_coordinate);
    f_middle = Target_function(start_coordinate + middle*search_direction_given);
    f_right = Target_function(start_coordinate + right*search_direction_given);
    ...
    //first choose a appropriate initial bracket with a convex-like shape
    {
        ...
        for(find_shape = 0; find_shape < find_shape_max; ++find_shape)
        {
            ...
            else if( (f_right < f_middle) )
            {
                ...
                f_right = f_right = Target_function(start_coordinate + right*search_direction_given);
            }
            else
            {
                break;
    ...
    while( (right - left)>=epsilon_scaled && count_iteration < _max_iteration)
    {
        if(f_left >= f_right)
        {
            ...
            f_next = Target_function(start_coordinate + next*search_direction_given);
